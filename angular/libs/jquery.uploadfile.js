...
(function($) {
  ...
  $.fn.uploadFile = function(options) {
    ...
    var s = $.extend({
      ...
      dragDropRegionClass: "ajax-upload-dragdrop",
      statusBarClass: 'media-cont ajax-uploading-file',
    ...
    this.uploadProgressContainer = (function() {
      if (s.sectionView && s.uploadContainer) {
        return s.sectionView.$(s.uploadContainer);
      } else if (s.uploadContainer) {
        return $(s.uploadContainer);
      } else {
        return $('<div class="upload-container"></div>');
      }
    }());
    this.responses = [];
    ...
    var uploadLabel = $(document.createElement(s.uploadButtonTagName)).append($(this).html());
    ...
    (function checkAjaxFormLoaded() {
      if ($.fn.ajaxForm) {
        ...
        if (s.dragDrop) {
          var dragDrop = $('<div class="' + s.dragDropRegionClass + '" style="vertical-align:top;"></div>');
          $(obj).before(dragDrop);
    ...
    function setDragDropHandlers(obj, s, ddObj) {
      ...
      ddObj.on('drop', function(e) {
        ...
        if (!s.multiple && files.length > 1) {
          if (s.showError) {
            showError(s.multiDragErrorStr, obj.errorLog);
          }
          return;
        }
    ...
    function serializeAndUploadFiles(s, obj, files) {
      for (var i = 0; i < files.length; i++) {
        if (!isFileTypeAllowed(obj, s, files[i].name)) {
          if (s.showError) {
            showError(files[i].name + ' ' + s.extErrorStr + s.allowedTypes, obj.errorLog);
          } //$("<div style='color:red;'> + "</div>").appendTo(obj.errorLog);
          continue;
        }
        if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
          if (s.showError) {
            showError(files[i].name + ' ' + s.sizeErrorStr + getSizeStr(s.maxFileSize), obj.errorLog);
        ...
        pd.statusbar.attr('data-filename', files[i].name);
      ...
      fileInput.change(function() {
        ...
        if (this.files) //support reading files
        {
          ...
          if (!isFileTypeAllowed(obj, s, filenameStr)) {
            if (s.showError) {
              showError(filenameStr + ' ' + s.extErrorStr + s.allowedTypes, obj.errorLog);
    ...
    function createProgressDiv(obj, s) {
      ...
      this.statusbar = $("<div class='" + (s.statusBarClass || 'ajax-file-upload-statusbar') + "'></div>");
      this.filename = $("<div class='ajax-file-upload-filename'></div>").appendTo(this.statusbar);
      this.progressDiv = $("<div class='ajax-file-upload-progress'>").appendTo(this.statusbar).hide();
      this.progressbar = $("<div class='ajax-file-upload-bar " + obj.formGroup + "'></div>").appendTo(this.progressDiv);
      this.abort = $('<button type="button" class="close ' + obj.formGroup + '" title="Abort upload">&times;</button>').prependTo(this.statusbar); //.hide();
      ...
      me.uploadProgressContainer.append(this.statusbar);
      return this;
    }
    ...
    function ajaxFormSubmit(form, s, pd, fileArray, obj) {
      ...
      var options = {
        ...
        error: function(xhr, status, errMsg) {
          ...
          console.log('Upload error = ', xhr.statusText, xhr)
    ...
    function showError(error, parentEl) {
      if (s.util) {
        s.util.showSectionMessage({
          view: s.sectionView,
          message: error
        });
      } else {
        $("<div style='color:red;'>" + error + "</div>").appendTo(parentEl);
      }
    }
